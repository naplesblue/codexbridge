...
当前测试显示，Free / Go 账号不暴露 CodexBridge 需要的 App 创建流程。

CodexBridge 不解锁 Developer Mode，不解锁模型，不绕过账号限制，也不提供账号访问。它只连接你自己的 ChatGPT App 界面和你自己的本地仓库。
...
不要安装未 scoped 的 `codexbridge` 包。那个 npm 包名属于另一个 CLI，本项目的标准安装方式是 scoped 包。
...
`npx -y -p @naplesblue/codexbridge codexbridge start` 仍然可用，但普通用户更容易理解全局安装。
...
Server URL: 粘贴 CodexBridge 复制的 URL
Authentication: No Authentication / None
```

复制的 Server URL 已经包含私有 CodexBridge token。

## CSP 要保持开启吗？

要保持开启。

CodexBridge 的小组件按 CSP 开启的路径构建。它不需要远程脚本、外部字体、iframe、第三方图片或任意外部请求。

## CodexBridge 会绕过速率限制吗？

不会。

CodexBridge 不绕过、不提升、不合并、不转售、不修改 ChatGPT、Codex、OpenAI 或第三方模型限制。所有请求仍然通过你自己的 ChatGPT 会话，并受该账号当前限制约束。

它的价值在于 ChatGPT 和 Codex 是不同产品界面。某个工作流暂时不可用时，如果另一个你本来就有权限的界面仍可用，CodexBridge 可以让它继续操作同一个本地仓库。

## CodexBridge 可以使用 GPT-5.5 吗？

前提是你的 ChatGPT 账号已经在 Web 产品里提供这个模型或同级更强模型，并且该模型界面可以调用 Developer Mode Apps。

CodexBridge 不提供、不代理、不转售、也不解锁模型。它只给兼容的 ChatGPT 会话提供本地仓库工具。
...
Plus / Pro 可以暴露 Apps / Developer Mode，但某个具体模型界面仍然可能不能调用连接器或 MCP 工具。遇到这种情况时，用 `codexbridge pro-bundle --copy` 导出上下文，再把计划交给本地代理执行。

## ChatGPT 能通过 CodexBridge 看到什么？
...
## CodexBridge 能把 bash 绑定到某个会话 id 吗？

CodexBridge 不能附加到、读取或复用某一个 Codex App 聊天会话或终端会话。

MCP 的 `bash` 工具是在你启动的 CodexBridge 本地服务器进程里，针对配置的 workspace root 执行。MCP session id 只是 ChatGPT 和 CodexBridge HTTP 服务器之间的传输状态，不是 Codex 会话 id。

但 CodexBridge 可以要求 bash 调用带上匹配的本地 session 标签：

```bash
codexbridge start --bash-session main --require-bash-session
```

之后 `bash` 调用必须包含 `session_id: "main"`。这能避免误触发到错误的 CodexBridge 终端，但不是远程控制某个已有的 Codex App 聊天。

如果你显式开启，CodexBridge 可以列出本地 Codex session id 和标题：
...
大多数用户建议用 ngrok free dev domain。创建免费 ngrok 账号，在 Universal Gateway -> Domains 找到分配给你的 dev domain，并在 `codexbridge setup` 里保存。

如果你有自己的域名，用 Cloudflare named tunnel，把 DNS 路由到例如 `codexbridge.example.com` 的主机名。
...
同一个 hostname 和 CodexBridge token 会被当前工作区复用。
...
分别在两个仓库里运行 `codexbridge setup` 并保存 profile。

## 为什么使用 naplesblue.github.io/codexbridge？

GitHub Pages 的 `owner.github.io` 只能由名为 `owner` 的 GitHub 用户或组织使用。

对这个 fork 来说，干净的项目 Pages 地址是：
...
```text
https://naplesblue.github.io/codexbridge/zh.html
```

## CodexBridge 是否违反服务条款？

CodexBridge 使用 ChatGPT 的官方 Developer Mode / MCP App 接入路径，让你自己的 ChatGPT 会话连接到你自己的本地工具。
...
## CodexBridge 能帮助 ChatGPT 维持上下文吗？
...
- `codexbridge pro-bundle --copy` 给不能调用工具的模型生成上下文包。
