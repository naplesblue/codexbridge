...
Usage:
  ...
  codexbridge doctor
  codexbridge execute-handoff --agent opencode --model provider/model
  codexbridge watch-handoff --agent opencode --model provider/model
  codexbridge --root /path/to/repo
  ...
  node scripts/codexbridge.mjs --root /path/to/repo --tunnel cloudflare
...
Options:
  ...
  --widget-domain <origin>   Dedicated HTTPS origin for ChatGPT widget iframes.
                             Required for app submission. Default: https://naplesblue.github.io.
  ...
  --cloudflared <path>      cloudflared executable. Default: PATH, then ~/.codexbridge/bin.
  --ngrok <path>            ngrok executable. Default: PATH.
  --ngrok-config <path>     Optional ngrok config file path.
  --no-profile              Do not load a saved ~/.codexbridge workspace profile.
  ...
  --install-cloudflared     Install/reinstall cloudflared into ~/.codexbridge/bin.
...
Execute handoff options:
  codexbridge execute-handoff --agent opencode --model provider/model
  codexbridge execute-handoff --agent pi --model provider/model
  codexbridge execute-handoff --agent custom --command "my-agent --task-file {{plan_file}}"
...
Watch handoff options:
  codexbridge watch-handoff --agent opencode --model provider/model
  codexbridge watch-handoff --agent pi --model provider/model
  codexbridge watch-handoff --agent custom --command "my-agent --task-file {{plan_file}}"
...
Ngrok stable URL mode:
  codexbridge ngrok --root /path/to/repo --hostname your-domain.ngrok-free.dev

Planning-only handoff mode:
  codexbridge start --root /path/to/repo --mode handoff

Execute a local handoff after ChatGPT writes .ai-bridge/current-plan.md:
  codexbridge execute-handoff --agent opencode --model provider/model
  codexbridge execute-handoff --agent pi --model provider/model
  codexbridge execute-handoff --agent custom --command "node ./agent.js --task-file {{plan_file}}" --yes

Watch for new handoff plans and execute them locally:
  codexbridge watch-handoff --agent opencode --model provider/model --yes
  codexbridge watch-handoff --agent custom --command "node ./agent.js --task-file {{plan_file}}" --yes

Stable URL mode after one-time Cloudflare tunnel setup:
  codexbridge stable --root /path/to/repo --hostname codexbridge.example.com --tunnel-name codexbridge
`);
}
...
function printSavedProfileHint(profile) {
  ...
  printBox('Saved setup found', [
    summary,
    'From this folder, future launches only need: codexbridge start',
    'Use codexbridge setup when you want to change the port, mode, tool mode, tunnel, hostname, or token.'
  ]);
}
...
function saveRuntimeConnection(root, details, options = {}) {
  ...
  const payload = {
    ...
    localStatusUrl: details.localStatusUrl ? details.localStatusUrl.replace(/codexbridge_token=[^&]+/, 'codexbridge_token=<redacted>') : '',
...
function bashSessionOptions(args, profile = {}) {
  const bashSession = validateBashSession(optionValue(args, profile, 'bashSession', ['CODEXBRIDGE_BASH_SESSION_ID'], ''));
  const requireBashSession = optionBool(args, profile, 'requireBashSession', ['CODEXBRIDGE_REQUIRE_BASH_SESSION'], false);
...
function bashTranscriptOption(args, profile = {}) {
  const value = optionValue(args, profile, 'bashTranscript', ['CODEXBRIDGE_BASH_TRANSCRIPT'], 'compact');
...
function codexSessionsOption(args, profile = {}) {
  const value = optionValue(args, profile, 'codexSessions', ['CODEXBRIDGE_CODEX_SESSIONS'], 'off');
...
async function downloadFile(url, destination) {
  const response = await fetch(url, {
    headers: { 'user-agent': 'codexbridge-launcher' }
  });
  if (!response.ok) {
...
async function installCloudflaredLocal() {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'codexbridge-cloudflared-'));
  const url = `https://github.com/cloudflare/cloudflared/releases/latest/download/${asset.file}`;

  fs.mkdirSync(binDir, { recursive: true, mode: 0o700 });
  console.error(`[codexbridge] Installing cloudflared locally: ${installPath}`);
  console.error(`[codexbridge] Downloading official Cloudflare release: ${asset.file}`);
  ...
  try {
    ...
    console.error('[codexbridge] cloudflared installed successfully.');
    return installPath;
  } finally {
...
async function resolveCloudflared(args) {
  ...
  if (!args.installCloudflared && commandExists('cloudflared')) {
    try {
      ...
      console.error(`[codexbridge] cloudflared in PATH failed --version; trying local install. ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  if (!args.installCloudflared && executableFileExists(localPath)) {
    try {
      ...
      console.error(`[codexbridge] Existing ${localPath} failed --version; reinstalling. ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
function ngrokConfigPath(args) {
  const configPath = args.ngrokConfig ?? process.env.NGROK_CONFIG ?? envValue('CODEXBRIDGE_NGROK_CONFIG') ?? '';
...
function portInUseHelp(host, port) {
  return [
    ...
    '',
    'Example:',
    '  repo A: codexbridge setup  -> port 8787 -> hostname A',
    '  repo B: codexbridge setup  -> port 8788 -> hostname B',
    '',
    'For quick tunnels you can also start the second repo with:',
    '  codexbridge start --port 8788',
    '',
...
function spawnLogged(name, command, args, options = {}) {
  ...
  child.codexbridgeLogTail = () => logLines.join('\n');
...
function endpointWithToken(endpoint, token) {
  ...
  url.searchParams.set('codexbridge_token', token);
  return url.toString();
}
...
function normalizeMode(args) {
  const mode = args.mode ?? envValue('CODEXBRIDGE_MODE') ?? 'agent';
...
function contextDirFromArgs(args) {
  return args.contextDir ?? envValue('CODEXBRIDGE_CONTEXT_DIR') ?? '.ai-bridge';
}
...
function buildExecutorCommand(args, root, planPath, planText) {
  ...
  const model = String(args.model ?? envValue('CODEXBRIDGE_AGENT_MODEL') ?? '').trim();
  const replacements = {
    model,
    plan_file: planPath,
    plan_text: planText,
...
function runProcessCaptured(command, args, options) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (exitCode, signal) => {
      ...
      const err = trimBytes(`${stderr}${timedOut ? `\n[codexbridge] Command timed out after ${timeoutMs} ms.` : ''}`, maxOutputBytes);
      resolve({
        exitCode,
        signal,
        durationMs: Date.now() - started,
...
async function confirmLocalExecution(args, root, commandInfo) {
  ...
  printBox('Confirm local execution', [
    ...
    'This runs a local process in the workspace. CodexBridge will collect status, logs, and git diff into .ai-bridge.'
  ]);
...
function loadHandoffExecution(args) {
  const root = realDir(args.root ?? envValue('CODEXBRIDGE_ROOT') ?? process.cwd());
  const contextDir = contextDirFromArgs(args);
  const bridgeDir = resolveWorkspaceFile(root, contextDir);
  const planPath = path.join(bridgeDir, 'current-plan.md');
  const maxReadBytes = numberOption(envValue('CODEXBRIDGE_MAX_READ_BYTES'), 180_000, 4_000, 2_000_000);
  const maxOutputBytes = numberOption(args.maxOutputBytes ?? envValue('CODEXBRIDGE_MAX_OUTPUT_BYTES'), 120_000, 4_000, 2_000_000);
...
function printHandoffDryRun(request, title = 'CodexBridge execute-handoff dry run') {
  printBox(title, [
...
async function runWatchHandoff(argv) {
  ...
  printBox('CodexBridge watch-handoff', [
    labelValue('Workspace', root),
  ...
  while (!stopped) {
    ...
    if (args.dryRun) {
      printHandoffDryRun(request, 'CodexBridge watch-handoff dry run');
...
function printConnectorBlock(endpoint, token, options = {}) {
  ...
  console.log('');
  console.log(paint('bold', 'CodexBridge ready'));
...
function printControlHelp() {
  ...
  console.log('  q      stop CodexBridge');
  console.log('');
}
...
function printModeHelp() {
  console.log('');
  console.log('Modes');
  console.log('  codexbridge start                 agent mode: read/write/edit/search/bash');
  console.log('  codexbridge start --no-bash       agent mode without ChatGPT-triggered shell commands');
  console.log('  codexbridge start --bash-session main --require-bash-session');
  console.log('  codexbridge start --mode handoff  planning-only .ai-bridge handoff');
  console.log('  codexbridge start --mode pro      export context for models without MCP tools');
  console.log('  codexbridge start --tool-mode minimal   expose only the tight coding loop');
  console.log('  codexbridge start --tool-mode full      expose every advanced compatibility tool');
  console.log('');
}
...
function printStableUrlHelp() {
  ...
  console.log('  codexbridge install-cloudflared');
  console.log('  ~/.codexbridge/bin/cloudflared tunnel login');
  console.log('  ~/.codexbridge/bin/cloudflared tunnel create codexbridge');
  console.log('  ~/.codexbridge/bin/cloudflared tunnel route dns codexbridge codexbridge.example.com');
  console.log('');
  console.log('Daily start:');
  console.log('  codexbridge stable --hostname codexbridge.example.com --tunnel-name codexbridge --token keep-this-stable-token');
  console.log('');
  console.log('Ngrok alternative with a reserved domain:');
  console.log('  ngrok config add-authtoken <your-ngrok-token>');
  console.log('  codexbridge ngrok --hostname your-domain.ngrok-free.dev --token keep-this-stable-token');
  console.log('');
}
...
async function runDoctor(argv) {
  ...
  const tunnel = optionValue(args, profile, 'tunnel', ['CODEXBRIDGE_TUNNEL'], 'cloudflare');
  const host = optionValue(args, profile, 'host', ['CODEXBRIDGE_HOST'], '127.0.0.1');
  const port = String(optionValue(args, profile, 'port', ['CODEXBRIDGE_PORT'], '8787'));
  const mode = optionValue(args, profile, 'mode', ['CODEXBRIDGE_MODE'], 'agent');
  const bash = optionValue(args, profile, 'bash', ['CODEXBRIDGE_BASH_MODE'], 'safe');
  const write = optionValue(args, profile, 'write', ['CODEXBRIDGE_WRITE_MODE'], mode === 'agent' ? 'workspace' : 'handoff');
  const toolMode = optionValue(args, profile, 'toolMode', ['CODEXBRIDGE_TOOL_MODE'], 'standard');
  ...
  if (tunnel === 'none') {
    ...
    record(cloudflaredPath ? 'ok' : 'warn', 'cloudflared', cloudflaredPath || 'missing now; codexbridge start can auto-install unless --no-install-cloudflared is used');
  } else if (tunnel === 'cloudflare-named') {
    record(stableHostname ? 'ok' : 'fail', 'Hostname', stableHostname || 'required for Cloudflare stable mode');
    record(cloudflaredPath ? 'ok' : 'warn', 'cloudflared', cloudflaredPath || 'missing now; run codexbridge install-cloudflared or pass --cloudflared');
    record(
      optionValue(args, profile, 'tunnelName', ['CLOUDFLARE_TUNNEL_NAME', 'CODEXBRIDGE_TUNNEL_NAME'], '') ||
        optionValue(args, profile, 'cloudflareTokenFile', ['CLOUDFLARE_TUNNEL_TOKEN_FILE', 'CODEXBRIDGE_CLOUDFLARE_TUNNEL_TOKEN_FILE'], '') ||
        optionValue(args, profile, 'cloudflareConfig', ['CLOUDFLARE_TUNNEL_CONFIG', 'CODEXBRIDGE_CLOUDFLARE_CONFIG'], '') ||
        optionValue(args, profile, 'cloudflareToken', ['CLOUDFLARE_TUNNEL_TOKEN', 'CODEXBRIDGE_CLOUDFLARE_TUNNEL_TOKEN'], '')
        ? 'ok'
        : 'fail',
      'Cloudflare setup',
...
async function collectTunnelPreference(rl, defaults, profile, options = {}) {
  ...
  if (tunnel === 'ngrok') {
    hostname = await ask(
      ...
      optionValue(defaults, profile, 'hostname', ['CODEXBRIDGE_PUBLIC_HOSTNAME', 'CODEXBRIDGE_HOSTNAME', 'NGROK_DOMAIN'], '')
    );
    if (!hostname) throw new Error('Ngrok setup needs your reserved domain, for example name.ngrok-free.dev.');
    ngrokConfig = optionValue(defaults, profile, 'ngrokConfig', ['NGROK_CONFIG', 'CODEXBRIDGE_NGROK_CONFIG'], '');
  } else if (tunnel === 'cloudflare-named') {
    hostname = await ask(
      rl,
      'Stable Cloudflare hostname, without /mcp',
      optionValue(defaults, profile, 'hostname', ['CODEXBRIDGE_PUBLIC_HOSTNAME', 'CODEXBRIDGE_HOSTNAME'], '')
    );
    if (!hostname) throw new Error('Stable public URL setup needs a real hostname, for example codexbridge.yourdomain.com.');
    tunnelName = await ask(rl, 'Cloudflare tunnel name', optionValue(defaults, profile, 'tunnelName', ['CODEXBRIDGE_TUNNEL_NAME', 'CLOUDFLARE_TUNNEL_NAME'], 'codexbridge'));
    cloudflareConfig = optionValue(defaults, profile, 'cloudflareConfig', ['CODEXBRIDGE_CLOUDFLARE_CONFIG', 'CLOUDFLARE_TUNNEL_CONFIG'], '');
    cloudflareTokenFile = optionValue(defaults, profile, 'cloudflareTokenFile', ['CODEXBRIDGE_CLOUDFLARE_TUNNEL_TOKEN_FILE', 'CLOUDFLARE_TUNNEL_TOKEN_FILE'], '');
  }
...
function profileFromPreference(root, args, profile, preference) {
  const mode = optionValue(args, profile, 'mode', ['CODEXBRIDGE_MODE'], 'agent');
  const port = String(optionValue(args, profile, 'port', ['CODEXBRIDGE_PORT'], '8787'));
  const bash = optionValue(args, profile, 'bash', ['CODEXBRIDGE_BASH_MODE'], '');
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
  const codexDir = optionValue(args, profile, 'codexDir', ['CODEXBRIDGE_CODEX_DIR'], '');
  const { bashSession, requireBashSession } = bashSessionOptions(args, profile);
  const write = optionValue(args, profile, 'write', ['CODEXBRIDGE_WRITE_MODE'], '');
  const toolMode = optionValue(args, profile, 'toolMode', ['CODEXBRIDGE_TOOL_MODE'], '');
  const widgetDomain = optionValue(args, profile, 'widgetDomain', ['CODEXBRIDGE_WIDGET_DOMAIN'], '');
  const existingToken = optionValue(args, profile, 'token', ['CODEXBRIDGE_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], '');
...
async function maybeConfigureFirstRun(root, args, profile) {
  ...
  if (reusableProfiles.length) {
    ...
    printBox('Saved setups', [
      'No saved settings exist for this workspace, but CodexBridge found saved setups from other workspaces.',
    ...
    try {
      ...
      if (Number.isInteger(selectedIndex) && selectedIndex >= 1 && selectedIndex <= shown.length) {
        ...
        const payload = reusableProfilePayload(selected, {
          port: String(optionValue(args, selected, 'port', ['CODEXBRIDGE_PORT'], selected.port ?? '8787')),
          mode: optionValue(args, selected, 'mode', ['CODEXBRIDGE_MODE'], selected.mode ?? 'agent')
        });
  ...
  printBox('First run setup', [
    ...
    'Choose once now. CodexBridge will reuse this choice on future codexbridge start runs until you change or delete it with codexbridge settings.'
  ]);
...
function commandPreview(args) {
  return ['codexbridge', ...args].map((part) => {
...
async function runSetupWizard(argv) {
  if (!process.stdin.isTTY) {
    throw new Error('codexbridge setup needs an interactive terminal. Use codexbridge start --root /path/to/repo for non-interactive scripts.');
  }
  const defaults = parseArgs(argv);
  const defaultRoot = path.resolve(expandHome(defaults.root ?? envValue('CODEXBRIDGE_ROOT') ?? process.cwd()));

  printBox('CodexBridge setup', [
    'This wizard prepares a ChatGPT connector for the folder you choose.',
    'Press Enter to accept defaults. Stable tunnel choices are saved per workspace under ~/.codexbridge.'
  ]);
  ...
  try {
    ...
    const savedTunnel = optionValue(defaults, profile, 'tunnel', ['CODEXBRIDGE_TUNNEL'], 'cloudflare');
    const defaultTunnel = savedTunnel === 'cloudflare-named'
      ...
      : savedTunnel === 'ngrok'
        ...
        : savedTunnel === 'none'
          ? 'local'
          : 'quick';
    const defaultPort = String(optionValue(defaults, profile, 'port', ['CODEXBRIDGE_PORT'], '8787'));
    const defaultMode = normalizeSetupChoice(optionValue(defaults, profile, 'mode', ['CODEXBRIDGE_MODE'], 'agent'), ['agent', 'handoff', 'pro'], 'agent');

    const port = await ask(rl, 'Which local port should CodexBridge use?', defaultPort);
    ...
    printBox('Public URL', [
      ...
      'quick  = CodexBridge creates a Cloudflare quick tunnel for demos and local work.',
    ...
    const bash = optionValue(defaults, profile, 'bash', ['CODEXBRIDGE_BASH_MODE'], '');
    const bashTranscript = bashTranscriptOption(defaults, profile);
    const codexSessions = codexSessionsOption(defaults, profile);
    const codexDir = optionValue(defaults, profile, 'codexDir', ['CODEXBRIDGE_CODEX_DIR'], '');
    const write = optionValue(defaults, profile, 'write', ['CODEXBRIDGE_WRITE_MODE'], '');
    const toolMode = optionValue(defaults, profile, 'toolMode', ['CODEXBRIDGE_TOOL_MODE'], '');
    const widgetDomain = optionValue(defaults, profile, 'widgetDomain', ['CODEXBRIDGE_WIDGET_DOMAIN'], '');
    ...
    let profileToken = optionValue(defaults, profile, 'token', ['CODEXBRIDGE_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], '');
    ...
    if (tunnelChoice === 'local') {
      ...
      const tunnelName = await ask(rl, 'Cloudflare tunnel name', optionValue(defaults, profile, 'tunnelName', ['CODEXBRIDGE_TUNNEL_NAME', 'CLOUDFLARE_TUNNEL_NAME'], 'codexbridge'));
      profileTunnelName = tunnelName;
      args.push('--tunnel', 'cloudflare-named', '--hostname', hostname, '--tunnel-name', tunnelName);
      profileCloudflareConfig = optionValue(defaults, profile, 'cloudflareConfig', ['CODEXBRIDGE_CLOUDFLARE_CONFIG', 'CLOUDFLARE_TUNNEL_CONFIG'], '');
      profileCloudflareTokenFile = optionValue(defaults, profile, 'cloudflareTokenFile', ['CODEXBRIDGE_CLOUDFLARE_TUNNEL_TOKEN_FILE', 'CLOUDFLARE_TUNNEL_TOKEN_FILE'], '');
      ...
      const ngrokConfig = optionValue(defaults, profile, 'ngrokConfig', ['NGROK_CONFIG', 'CODEXBRIDGE_NGROK_CONFIG'], '');
      if (ngrokConfig) {
    ...
    if (profileTunnel !== 'none') {
      profileToken = await ask(rl, 'CodexBridge auth token for this workspace', stableToken(profileToken));
    ...
    const startAnswer = await ask(rl, 'Start CodexBridge now?', 'yes');
...
function printProfile(root, profile) {
  if (!profile.profilePath) {
    printBox('CodexBridge settings', [
      labelValue('Workspace', root),
      'No saved settings for this workspace.',
      'Run codexbridge settings set or codexbridge setup to save a tunnel preference.'
    ]);
    return;
  }
...
function printProfileList(profiles = listWorkspaceProfiles()) {
  if (!profiles.length) {
    printBox('CodexBridge saved setups', [
      'No saved workspace settings found.',
      'Run codexbridge setup or codexbridge settings set to create one.'
    ]);
    return;
  }
  printBox('CodexBridge saved setups', profiles.slice(0, 50).map((profile, index) => profileOneLine(profile, index + 1)));
}

function saveSettingsFromArgs(root, args, profile) {
  const tunnel = optionValue(args, profile, 'tunnel', ['CODEXBRIDGE_TUNNEL'], profile.tunnel ?? 'cloudflare');
  ...
  const mode = optionValue(args, profile, 'mode', ['CODEXBRIDGE_MODE'], profile.mode ?? 'agent');
  const toolMode = optionValue(args, profile, 'toolMode', ['CODEXBRIDGE_TOOL_MODE'], profile.toolMode ?? '');
  const widgetDomain = optionValue(args, profile, 'widgetDomain', ['CODEXBRIDGE_WIDGET_DOMAIN'], profile.widgetDomain ?? '');
  const port = String(optionValue(args, profile, 'port', ['CODEXBRIDGE_PORT'], profile.port ?? '8787'));
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
  const codexDir = optionValue(args, profile, 'codexDir', ['CODEXBRIDGE_CODEX_DIR'], profile.codexDir ?? '');
  const { bashSession, requireBashSession } = bashSessionOptions(args, profile);
  const token = tunnel === 'none'
    ? optionValue(args, profile, 'token', ['CODEXBRIDGE_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], profile.token ?? '')
    : stableToken(optionValue(args, profile, 'token', ['CODEXBRIDGE_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], profile.token ?? ''));
  const savedPath = saveWorkspaceProfile(root, {
    port,
    mode,
    tunnel,
...
async function runSettings(argv) {
  ...
  if (action === 'delete' || action === 'reset' || action === 'remove') {
    ...
    if (!args.yes && process.stdin.isTTY) {
      ...
      throw new Error('Use codexbridge settings delete --yes in non-interactive shells.');
    }
...
function writeControlPrompt() {
  process.stdout.write('codexbridge> ');
}
...
async function main() {
  ...
  if (profile.profilePath && !args.noProfile) {
    ...
    const summary = profileSummary(profile);
    if (summary) statusLine('ok', `${summary}. Future launches from this folder only need: codexbridge start`);
  }
  ...
  if (tunnel === 'ngrok' && !stableHostname) {
    throw new Error('--hostname is required with ngrok tunnel mode. Example: codexbridge ngrok --hostname your-domain.ngrok-free.dev');
  }
  if (args.noAuth && tunnel !== 'none') {
    throw new Error('--no-auth is only allowed with --tunnel none. Public tunnels require CODEXBRIDGE_HTTP_TOKEN.');
  }
  const mode = optionValue(args, profile, 'mode', ['CODEXBRIDGE_MODE'], 'agent');
  if (!['agent', 'handoff', 'pro'].includes(mode)) {
    throw new Error('--mode must be agent, handoff, or pro');
  ...
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
  const codexDir = resolveCodexDir(root, optionValue(args, profile, 'codexDir', ['CODEXBRIDGE_CODEX_DIR'], ''));
  const { bashSession, requireBashSession } = bashSessionOptions(args, profile);
  const write = optionValue(args, profile, 'write', ['CODEXBRIDGE_WRITE_MODE'], mode === 'agent' ? 'workspace' : 'handoff');
  const toolMode = optionValue(args, profile, 'toolMode', ['CODEXBRIDGE_TOOL_MODE'], 'standard');
  const widgetDomain = optionValue(args, profile, 'widgetDomain', ['CODEXBRIDGE_WIDGET_DOMAIN'], 'https://naplesblue.github.io');
  ...
  let token = args.noAuth ? '' : optionValue(args, profile, 'token', ['CODEXBRIDGE_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], '');
  if (!token && tunnel !== 'none') token = stableToken();

  const serverEnv = {
    ...process.env,
    CODEXBRIDGE_ROOT: root,
    CODEXBRIDGE_ALLOWED_ROOTS: allowRoots.join(path.delimiter),
    CODEXBRIDGE_HOST: host,
    CODEXBRIDGE_PORT: port,
    CODEXBRIDGE_BASH_MODE: bash,
    CODEXBRIDGE_BASH_TRANSCRIPT: bashTranscript,
    CODEXBRIDGE_BASH_SESSION_ID: bashSession,
    CODEXBRIDGE_REQUIRE_BASH_SESSION: requireBashSession ? '1' : '0',
    CODEXBRIDGE_CODEX_SESSIONS: codexSessions,
    CODEXBRIDGE_WRITE_MODE: write,
    CODEXBRIDGE_TOOL_MODE: toolMode,
    CODEXBRIDGE_WIDGET_DOMAIN: widgetDomain,
    CODEXBRIDGE_MODE: mode,
    CODEXBRIDGE_TUNNEL_MODE: tunnel === 'none' ? '0' : '1'
  };
  if (codexDir) serverEnv.CODEXBRIDGE_CODEX_DIR = codexDir;
  if (args.logRequests || envValue('CODEXBRIDGE_LOG_REQUESTS') === '1') serverEnv.CODEXBRIDGE_LOG_REQUESTS = '1';
  if (args.allowHome) serverEnv.CODEXBRIDGE_ALLOW_HOME = '1';
  if (token) serverEnv.CODEXBRIDGE_HTTP_TOKEN = token;
  else delete serverEnv.CODEXBRIDGE_HTTP_TOKEN;

  if (args.printEnv) {
    console.log(JSON.stringify({ ...serverEnv, CODEXBRIDGE_HTTP_TOKEN: token ? '<redacted>' : undefined }, null, 2));
  }
  ...
  const verboseLogs = Boolean(args.logRequests || envValue('CODEXBRIDGE_LOG_REQUESTS') === '1');
  statusLine('wait', 'Starting local MCP server');
  const server = spawnLogged('codexbridge', process.execPath, [httpPath], { cwd: projectRoot, env: serverEnv, verbose: verboseLogs });
  let cloudflared;
  ...
  if (tunnel === 'ngrok') {
    ...
    try {
      ...
      const tail = typeof cloudflared.codexbridgeLogTail === 'function' ? cloudflared.codexbridgeLogTail() : '';
      const hint = [
        '',
        ...
        '  codexbridge ngrok --hostname your-domain.ngrok-free.dev --token keep-this-stable-token',
        '',
  ...
  const publicBase = publicBaseFromHostname(stableHostname);
  const tunnelName = optionValue(args, profile, 'tunnelName', ['CLOUDFLARE_TUNNEL_NAME', 'CODEXBRIDGE_TUNNEL_NAME'], '');
  const cloudflareConfig = optionValue(args, profile, 'cloudflareConfig', ['CLOUDFLARE_TUNNEL_CONFIG', 'CODEXBRIDGE_CLOUDFLARE_CONFIG'], '');
  const cloudflareTokenFile = optionValue(args, profile, 'cloudflareTokenFile', ['CLOUDFLARE_TUNNEL_TOKEN_FILE', 'CODEXBRIDGE_CLOUDFLARE_TUNNEL_TOKEN_FILE'], '');
  const cloudflareToken = optionValue(args, profile, 'cloudflareToken', ['CLOUDFLARE_TUNNEL_TOKEN', 'CODEXBRIDGE_CLOUDFLARE_TUNNEL_TOKEN'], '');
...
main().catch((error) => {
  ...
  if (envValue('CODEXBRIDGE_DEBUG') === '1' && error instanceof Error && error.stack) {
