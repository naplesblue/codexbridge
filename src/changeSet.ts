...
export function changesFromUnifiedDiff(input: UnifiedDiffChangeInput): ChangeSetInput[] {
  ...
  if (!diff.trim()) throw new CodexBridgeError("unified_diff must not be empty.");
  ...
  function flushHunk(): void {
    ...
    if (!currentPath) throw new CodexBridgeError("unified_diff is missing a target path. Provide path or include a +++ b/path header.");
    if (!oldText) throw new CodexBridgeError(`unified_diff for ${currentPath} cannot be converted because the hunk has no removable/context text.`);
    if (oldText !== newText) {
      changes.push({
        path: currentPath,
        old_text: oldText,
  ...
  for (const line of diff.split("\n")) {
    ...
    if (line.startsWith("+++ ")) {
      ...
      if (!currentPath) throw new CodexBridgeError("Creating or deleting files from unified_diff is not supported. Use explicit change set writes instead.");
      continue;
    }
    if (line.startsWith("@@")) {
      flushHunk();
    ...
    if (line.startsWith(" ")) {
      ...
      throw new CodexBridgeError(`Unsupported unified_diff line for ${currentPath || "unknown path"}: ${line.slice(0, 80)}`);
    }
  }
  flushHunk();

  if (!changes.length) throw new CodexBridgeError("unified_diff did not contain any applicable text changes.");
  return changes;
}

async function readExistingText(config: CodexBridgeConfig, guard: PathGuard, absPath: string): Promise<{ text: string; existed: boolean }> {
  try {
...
function assertBaseSha(change: ChangeSetInput, relPath: string, before: string, existed: boolean): string | null {
  ...
  if (change.base_sha256 && change.base_sha256 !== baseSha) {
    throw new CodexBridgeError(`base_sha256 mismatch for ${relPath}. Expected ${change.base_sha256}, current ${baseSha ?? "missing"}.`);
  }
  return baseSha;
}
...
async function prepareChange(
  ...
  if (isEditChange(rawChange)) {
    ...
    if (!before.existed) throw new CodexBridgeError(`Cannot edit missing file: ${resolved.relPath}`);
    ...
    if (typeof rawChange.expected_replacements === "number" && replacements !== rawChange.expected_replacements) {
      throw new CodexBridgeError(`Expected ${rawChange.expected_replacements} replacements but would perform ${replacements}.`);
    }
  } else {
    kind = "write";
  ...
  if (hasSecretValue(nextText)) {
    throw new CodexBridgeError("Secret-looking content is blocked from change set. Use placeholders such as [REDACTED_SECRET].");
  }
...
export async function previewChangeSet(
  ...
  if (!Array.isArray(rawChanges) || rawChanges.length === 0) {
    throw new CodexBridgeError("changes must contain at least one change.");
  }
  if (rawChanges.length > 50) {
    throw new CodexBridgeError("changes is limited to 50 entries.");
  }
